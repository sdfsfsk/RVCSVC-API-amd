...
if exist "%NATIVE_PYTHON%" (
    ...
    echo [SVC] Checking native AMD ROCm runtime...
    ...
    echo [SVC] Starting gateway with native AMD ROCm 7.2.1 + PyTorch 2.9.1...
    "%NATIVE_PYTHON%" "%~dp0app_svc.py" --is_nohalf
) else if exist "%LEGACY_PYTHON%" (
    echo [WARN] Native ROCm runtime not found; falling back to legacy DirectML...
    set "RVC_USE_DML=1"
    "%LEGACY_PYTHON%" "%~dp0app_svc.py" --dml --is_nohalf
) else (
